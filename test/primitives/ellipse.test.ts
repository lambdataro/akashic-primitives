...
describe("rectangle", () => {
	test("楕円を1つ描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "ellipse1", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "ellipse1.png"));
	...
	test("楕円をいろいろな大きさで描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "ellipse2", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "ellipse2.png"));
	...
	test("点・水平線・垂直線になる場合に描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "ellipse3", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
