...
describe("filledPolygon", () => {
	test("塗りつぶしたポリゴンを描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "filledPolygon", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "filledPolygon.png"));
