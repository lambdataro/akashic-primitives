...
describe("roundedRectangle", () => {
	test("塗りつぶした角丸長方形を描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "roundedBox", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "roundedBox.png"));
