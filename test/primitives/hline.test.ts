...
describe("hline", () => {
	test("水平線を描画できる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "hline1", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "hline.png"));
	...
	test("x1,x2を入れ替えできる", async () => {
		const context = new GameContext({
			gameJsonPath: path.join(__dirname, "hline2", "game.json"),
		});
		const client = await context.getGameClient({ renderingMode: "canvas" });
		const expectedImage = fs.readFileSync(path.join(__dirname, "..", "images", "hline.png"));
